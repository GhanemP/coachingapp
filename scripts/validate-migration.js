...
class MigrationValidator {
    ...
    async testDatabaseConnection() {
        try {
            ...
            this.addResult('testDatabaseConnection', 'failed', 'Cannot connect to PostgreSQL', error.message);
        }
    }
    ...
    async testTableStructure() {
        try {
            const tables = [
                'User', 'Team', 'Agent', 'Session', 
                'ActionItem', 'QuickNote', 'Scorecard', 'Notification'
            ];
            ...
            this.addResult('testTableStructure', 'failed', 'Table structure validation failed', error.message);
        }
    }
    ...
    async testDataCounts() {
        if (!fs.existsSync(SQLITE_DB_PATH)) {
            this.addResult('testDataCounts', 'warnings', 'SQLite database not found - skipping comparison');
            return;
        }
        ...
        try {
            ...
            if (discrepancies.length === 0) {
                this.addResult('testDataCounts', 'passed', 'Data counts match between SQLite and PostgreSQL', pgCounts);
            } else {
                this.addResult('testDataCounts', 'warnings', 'Data count discrepancies found', discrepancies);
            }
        } catch (error) {
    ...
    async testDataIntegrity() {
        try {
            ...
            this.addResult('testDataIntegrity', 'failed', 'Data integrity validation failed', error.message);
        }
    }
    ...
    async testBusinessLogic() {
        try {
            ...
            if (completedItemsWithoutDates.length > 0) {
                issues.push(`${completedItemsWithoutDates.length} completed action items without completion dates`);
            }
            ...
            this.addResult('testBusinessLogic', 'failed', 'Business logic validation failed', error.message);
        }
    }
    ...
    async testPerformance() {
        try {
            ...
            if (slowTests.length === 0) {
                this.addResult('testPerformance', 'passed', 'Performance validation passed', performanceTests);
            } else {
