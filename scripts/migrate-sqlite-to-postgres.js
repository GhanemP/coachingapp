...
async function createPostgresBackup() {
  ...
  try {
    ...
    await execPromise(
      `docker-compose exec -T postgres pg_dump -U coaching_user -d coaching_app > ${backupFile}`
    );
...
async function validateMigration(originalData) {
  ...
  try {
    ...
    Object.keys(counts).forEach(table => {
      ...
      console.log(
        `│ ${table.padEnd(15)} │ ${original.toString().padStart(8)} │ ${migrated.toString().padStart(8)} │   ${status}    │`
      );
