...
async function seedNewScorecardData() {
  try {
    ...
    for (const agent of agents) {
      console.log(
        `👤 Generating NEW scorecard data for ${agent.name} (${agent.agentProfile?.employeeId || 'No ID'})...`
      );
      ...
      for (const { month, year } of months) {
        ...
        const actualHours =
          scheduledHours * (basePerformance + variation + (Math.random() - 0.5) * 0.05);
        ...
        const tasksCompleted = Math.floor(
          tasksAssigned * (basePerformance + variation + (Math.random() - 0.5) * 0.1)
        );
        const expectedOutput = tasksCompleted * (80 + Math.random() * 40); // Expected units per task
        const actualOutput = Math.floor(
          expectedOutput * (basePerformance + variation + (Math.random() - 0.5) * 0.1)
        );
        ...
        const actualTimeSpent =
          standardTime / (basePerformance + variation + (Math.random() - 0.5) * 0.1);
        ...
        totalRecords++;
        console.log(
          `  ✅ Created NEW scorecard metric for ${month}/${year} (Score: ${Math.round(totalScore * 100) / 100}%)`
        );
        console.log(
          `     📊 Key metrics: Attendance ${Math.round(attendanceRate)}%, Tasks ${Math.round(taskCompletionRate)}%, Quality ${Math.round(qualityScore)}%`
        );
      }
    }
    ...
    console.log(
      `📅 Data spans ${months.length} months (${months[0].month}/${months[0].year} to ${months[months.length - 1].month}/${months[months.length - 1].year})`
    );
