...
async function main() {
  ...
  const permissions = [
    ...
    {
      name: 'conduct_sessions',
      description: 'Conduct coaching sessions',
      resource: 'sessions',
      action: 'execute',
    },
    ...
    // Scorecards
    {
      ...
      description: 'View scorecards',
      resource: 'scorecards',
      action: 'read',
    },
    {
      name: 'manage_scorecards',
    ...
    // Action items and plans
    {
      ...
      resource: 'action_items',
      action: 'write',
    },
    {
      name: 'view_action_items',
    ...
    // Quick notes
    {
      name: 'manage_quick_notes',
      description: 'Manage quick notes',
      resource: 'quick_notes',
      action: 'write',
    },
    {
      name: 'view_quick_notes',
      description: 'View quick notes',
      resource: 'quick_notes',
      action: 'read',
    },
  ...
  for (const agent of agents) {
    for (let month = 1; month <= 6; month++) {
      ...
      const tasksCompleted = Math.floor(
        tasksAssigned * (basePerformance + (Math.random() - 0.5) * 0.1)
      );
      const expectedOutput = tasksCompleted * (80 + Math.random() * 40);
      const actualOutput = Math.floor(
        expectedOutput * (basePerformance + (Math.random() - 0.5) * 0.1)
      );
